...
export const RackPage = () => {
    const { getRacksList, rackList, getAllFloor, allFloor, create_rack, edit_rack } = useMasterContext()
    ...
    useEffect(() => {
        if (rackList.data.length === 0 && rackList?.loading === true) {
            getRacksList()
        }
    }, [rackList.data])
    ...
    useEffect(() => {
        setState({...state, title: rackDetail?.title || '', type: rackDetail?.type || 'Sell'})
    }, [rackDetail])
                ...
                <Row>
                    <Col sm="12">
                        <Card>
                            ...
                            <CardBody>
                                <div className="btn-popup pull-right" onClick={() => { setIsEditing(false); setRackDetail(null); setIsOpen(true); }}>
                                ...
                                <div className="px-sm-3">
                                    <Table responsive hover borderless align="center">
                                        ...
                                        <tbody>
                                            {!rackList.loading && rackList?.data?.map((item, i) => (
                                                <tr key={i}>
                ...
                <ModalBody>
                    <Form onSubmit={onSubmit}>
                        ...
                        <FormGroup>
                            ...
                            <Input type="text" required min={3} placeholder="Enter rack name" name="title" onChange={onChange} value={state.title} disabled={isProcessing} />
                        </FormGroup>
                        {!isEditing && (
                            <FormGroup>
                        ...
                        <FormGroup className="m-checkbox-inline mb-0 custom-radio-ml d-flex radio-animated">
                            <Label className="d-block">
                                <Input 
                                    className="radio_animated" type="radio" disabled={isProcessing}
                                    onChange={(e) => setState({...state, type: e.target.checked ? 'Sell' : ''})} 
                            ...
                            <Label className="d-block mx-4">
                                <Input 
                                    className="radio_animated" disabled={isProcessing}
                                    type="radio"
                                    onChange={(e) => setState({...state, type: e.target.checked ? 'Lab' : ''})} 
