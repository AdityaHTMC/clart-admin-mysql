...
import { Badge, Button, Card, CardBody, Col, Container, Row, Table, Modal, ModalBody, ModalHeader, ModalFooter, Dropdown, DropdownToggle, DropdownMenu, DropdownItem } from "reactstrap";
...
export const ColonyList = () => {
    ...
    useEffect(() => {
        if (colonyList.data?.length === 0 && colonyList.loading === true) {
            getColonyList();
        }
    }, [colonyList.data]);
    ...
    return (
        <>
                            ...
                            <CardBody>
                                <div id="basicScenario" className="product-physical">
                                    <Table responsive hover borderless align="center">
                                        ...
                                        <tbody>
                                            {!colonyList.loading && colonyList?.data?.map((item, index) => (
                                                <tr key={index}>
                                                    <td>
                                                        <Badge>{item?.ref}</Badge>
                                                    </td>
                                                    <td className="text-center"> {item?.item_name || 'N/A'} </td>
                                                    <td className="text-center">
                                                        <Badge color="danger">{item?.breed_name || 'N/A'}</Badge>
                                                    </td>
                                                    <td className="text-center"> {item?.total_male || 0} </td>
                                                    <td className="text-center"> {item?.total_female || 0} </td>
                                                    <td className="text-center">
                                                    ...
                                                    <td>
                                                        <div className="d-flex justify-content-end align-items-center gap-2">
                                                            ...
                                                            <Dropdown isOpen={activeIndex === index} toggle={() => setActiveIndex(activeIndex === index ? -1 : index)} direction="down">
                                                                <DropdownToggle tag='button' color="danger" size="sm" className="p-0 m-0 btn " style={{ background: 'none' }} >
                                                                    <IoMdMore size={24} className="m-0 p-0" />
                                                                </DropdownToggle>
                                                                <DropdownMenu >

                                                                    <DropdownItem className="d-flex gap-2 align-items-center" onClick={() => { setQrOpen(true); setItemDetail(item); }}>
                                                                    ...
                                                                    {(item?.total_items === 0 || item?.children_total === 0) && (
                                                                        <DropdownItem className="d-flex gap-2 align-items-center" onClick={() => { setIsOpen(true); setItemDetail(item); }}>
                                                                            ...
                                                                            <span style={{ fontSize: 14, fontWeight: 500 }}>Add Items</span>
                                                                        </DropdownItem>
                                                                    )}

                                                                    {(item?.total_items > 0 || item?.children?.total > 0) && (
                                                                        <DropdownItem className="d-flex gap-2 align-items-center" onClick={() => { setIsRemove(true); setItemDetail(item); }}>
                                                                            <Badge color="warning" style={{ cursor: 'pointer' }}  >
                                                                                <FaMinus size={14} />
                                                                            </Badge>
                                                                            <span style={{ fontSize: 14, fontWeight: 500 }}>Remove Items</span>
                                                                        </DropdownItem>

                                                                    )}

                                                                    {(item?.total_items > 0 || item?.children?.total > 0) && (
                                                                        <DropdownItem className="d-flex gap-2 align-items-center" onClick={() => { setIsTransfer(true); setItemDetail(item); }} >
                                                                            <Badge color="danger" style={{ cursor: 'pointer' }}>
                                                                                <BiTransfer size={14} />
                                                                            </Badge>
                                                                            <span style={{ fontSize: 14, fontWeight: 500 }}>Transfer Item</span>
                                                                        </DropdownItem>
                                                                    )}
            ...
            <Modal isOpen={qrOpen} toggle={setQrOpen}>
                ...
                <ModalBody>
                    {itemDetail?.qr && (
                        <div className="d-flex justify-content-center" style={{ width: '100%', maxHeight: '200px' }}>
                            <img src={itemDetail?.qr} alt="" style={{ width: 'auto', height: '100%', aspectRatio: 1, objectFit: 'contain' }} />
                        </div>
                    )}
                </ModalBody>
                <ModalFooter>
